...
(function (global, lib) {
  ...
  const _select = function (selector) {
    const t = selector[0],
  ...
  const window2canvas = function (canvas, x, y) {
  ...
  _martix.prototype = {
    createMatrix: function (x, y, scale, rotate) {
      this.offset.x += (x + this.offset.x * (scale - 1));       
      this.offset.y += (y + this.offset.y * (scale - 1));      

      this.scale *= scale;
      this.rotate += rotate;

      const m = this.matrix; // just to make it easier to type and read
      const im = this.inverseMatrix; // just to make it easier to type and read
  ...
  //异步读取数据
  //param:method(GET或POST)  url(文件路径或者.aspx文件路径/方法名) dataType:'txt'||'json'  param(json{par1:val1,par2,val2...})
  const _ajax = function (param, fnSuccess, fnFaild) {
    //1、创建ajax对象
    let oAjax = null;
    ...
    //2、建立连接
    oAjax.open(param.method, param.url, true);
    //if (param.contentType) {
    //    oAjax.setRequestHeader('Content-Type', param.contentType);
    //}
    //3、发送请求
    oAjax.send(param.param);
    //4.接收数据
    oAjax.onreadystatechange = function () {
      if (oAjax.readyState == 4) {
        if (oAjax.status == 200) { 
          if (param.dataType.toLowerCase() == 'json')
  ...
  class _map {
    constructor(idName, coordType, unit) {
  ...
  _map.prototype = {
    ...
    fullExtent: function () {
      this.graphic.fullExtent(this.extent);
